...
После `docker compose up -d` dashboard доступен на порту `443` через nginx (HTTPS, самоподписанный сертификат).

```bash
open "https://your.server/"
```

Браузер покажет предупреждение о сертификате — это нормально для самоподписанного SSL.
