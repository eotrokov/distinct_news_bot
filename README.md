...
Telegram-бот **SEO-дайджеста** из ваших публичных каналов и RSS-лент: без дублей, рекламы и оффтопа.
...
Источники: публичные Telegram-каналы (`@channel` / `https://t.me/channel` / папка `t.me/addlist/…`) и RSS/Atom-ленты (`https://site.com/feed/`). Telegram читается через публичный превью `t.me/s/...`.

## Команды

- `/start`, `/help` — справка
- `/add @channel [название]` — добавить Telegram-канал
- `/add rss https://site.com/feed/ [название]` — добавить RSS/Atom-ленту
...
- При первом `/start` без источников бот просит прислать 1–3 @channel или RSS URL и сразу делает пробную сводку
...
2. Админ: `/start` → `/add @channel` или `/add rss https://site.com/feed/` → `/news`.
...
Или откройте «Источники» → «Готовые наборы» → «SEO RSS новости», чтобы добавить
10 SEO-лент из поставляемого набора: Ahrefs, Backlinko, Moz, Search Engine
Journal, Search Engine Land, Semrush, Google Search Central, Screaming Frog,
Aleyda Solis и Marie Haynes.
...
Дубли отсекаются по заголовку, URL и похожести текстов между Telegram и RSS. Оффтоп и реклама (курсы, вакансии, покупка ссылок) отфильтровываются. Сводка группирует по SEO-блокам и ранжирует по реакциям/просмотрам, а RSS-материалы без метрик идут после материалов с Telegram-реакциями.
...
Telegram-каналы должны быть **публичными** (доступен `https://t.me/s/<channel>`). RSS-ленты должны отдавать RSS 2.0 или Atom XML.
