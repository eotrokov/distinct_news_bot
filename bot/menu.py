...
async def show_main_menu(
    ...
    *,
    ...
    if not is_private_chat(update.effective_chat):
        text = (
        "Меню для этого чата. Источники и расписание общие для группы.\n"
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith("m:src_preset:"):
        ...
        channel_preset = get_channel_preset(slug)
        rss_preset = get_rss_preset(slug)
        if channel_preset:
            from bot.sources_ops import add_telegram_channels, format_add_report

            added, skipped = add_telegram_channels(
                db, chat_id, list(channel_preset.channels)
            )
            report = format_add_report(
                folder_title=channel_preset.title,
                added=added,
                skipped=skipped,
            )
...
async def on_awaiting_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        if kind == "onboard":
            ...
            from bot.sources_ops import add_from_text, format_add_report
            ...
            rss_urls = parse_rss_urls(text)
            if not handles and not rss_urls:
                await update.message.reply_text(
                    "Не нашёл источников. Пришлите @name, https://t.me/name "
                    "или RSS https://site.com/feed/\n"
                    "Или /cancel чтобы отменить.",
                    reply_markup=kb,
                )
                return

            added, skipped = add_from_text(db, chat_id, text)
            clear_awaiting(context)
            await update.message.reply_text(
        ...
        if kind == "source":
            ...
            handles = parse_telegram_handles(text)
            rss_urls = parse_rss_urls(text)
            if len(handles) + len(rss_urls) > 1 or (handles and rss_urls):
                added, skipped = add_from_text(db, chat_id, text)
                clear_awaiting(context)
                await update.message.reply_text(
                    format_add_report(
            ...
            clear_awaiting(context)
            kind_label = "RSS" if source.source_type == "rss" else "канал"
            await update.message.reply_text(
